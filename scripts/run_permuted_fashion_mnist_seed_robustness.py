...
from scripts.rank_permuted_fashion_mnist_sweeps import biomarker_score, build_markdown_report, primary_score
...
def summarize_seed_runs(seed_runs: list[dict]) -> dict:
    ...
    final_acc = values("permuted_fashion_mnist_final_accuracy_mean")
    forgetting = values("permuted_fashion_mnist_mean_forgetting")
    bwt = values("permuted_fashion_mnist_bwt")
    margins = values("permuted_fashion_mnist_mean_margin")
    ...
    summary["stable_across_seeds"] = bool(
        summary["final_accuracy"]["min"] >= 0.20
        and summary["forgetting"]["max"] <= 0.18
...
def build_seed_markdown(seed_runs: list[dict], summary: dict) -> str:
    lines = [
        "# Permuted-FashionMNIST Seed Robustness",
        "",
...
def run_seed_robustness(config_path: Path, seeds: tuple[int, ...]) -> dict:
    ...
    for seed in seeds:
        ...
        cfg.run.name = f"permuted_fashion_mnist_seed_{seed}"
...
def main() -> None:
    ...
    parser.add_argument("--json-out", default="artifacts/permuted_fashion_mnist_seed_robustness.json")
    parser.add_argument("--markdown-out", default="artifacts/permuted_fashion_mnist_seed_robustness.md")
