...
def _load_run_payload(run_dir: Path) -> dict | None:
    ...
    if assay_name != "split_fashion_mnist":
        return None
...
def primary_score(metrics: dict) -> float:
    return float(
        1.00 * metrics.get("split_fashion_mnist_final_accuracy_mean", 0.0)
        + 0.35 * metrics.get("split_fashion_mnist_peak_accuracy_mean", 0.0)
        - 0.75 * metrics.get("split_fashion_mnist_mean_forgetting", 0.0)
        + 0.35 * metrics.get("split_fashion_mnist_bwt", 0.0)
        + 0.10 * metrics.get("split_fashion_mnist_mean_margin", 0.0)
    )
...
def rank_runs(artifacts_root: Path) -> list[dict]:
    ...
    candidates.sort(
        key=lambda item: (item["primary_score"], item["biomarker_score"]),
        reverse=True,
    )
    return candidates
...
def build_markdown_report(ranked_runs: list[dict]) -> str:
    lines = [
        "# Split-FashionMNIST Sweep Ranking",
        "",
        "Primary metrics are ranked first; biomarkers are only secondary tie-breakers.",
        "",
    ]
    if not ranked_runs:
        lines.append("No Split-FashionMNIST runs were found.")
        return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    parser.add_argument("--json-out", default="artifacts/split_fashion_mnist_sweep_ranking.json")
    parser.add_argument("--markdown-out", default="artifacts/split_fashion_mnist_sweep_ranking.md")
