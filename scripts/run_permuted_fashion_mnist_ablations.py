...
from scripts.rank_permuted_fashion_mnist_sweeps import biomarker_score, primary_score


DEFAULT_CONDITIONS = tuple(MNISTSanityAssay.CONDITION_SPECS.keys())
...
def _persist_condition_run(cfg, result, *, condition_name: str) -> dict:
    ...
    cfg_dict["permuted_fashion_mnist_condition"] = condition_name
...
def _delta_metrics(baseline_metrics: dict, ablated_metrics: dict) -> dict[str, float]:
    baseline_accuracy = float(baseline_metrics.get("permuted_fashion_mnist_final_accuracy_mean", 0.0))
    baseline_peak = float(baseline_metrics.get("permuted_fashion_mnist_peak_accuracy_mean", 0.0))
    baseline_forgetting = float(baseline_metrics.get("permuted_fashion_mnist_mean_forgetting", 0.0))
    baseline_bwt = float(baseline_metrics.get("permuted_fashion_mnist_bwt", 0.0))
    baseline_margin = float(baseline_metrics.get("permuted_fashion_mnist_mean_margin", 0.0))
    ...
    baseline_z_gap = float(baseline_metrics.get("z_memory_alignment_gap", 0.0))
    baseline_growth_peak = float(baseline_metrics.get("permuted_fashion_mnist_peak_growth_pressure_mean", 0.0))
    baseline_growth_trigger = float(baseline_metrics.get("permuted_fashion_mnist_growth_trigger_crossed_fraction", 0.0))

    accuracy_drop = baseline_accuracy - float(ablated_metrics.get("permuted_fashion_mnist_final_accuracy_mean", 0.0))
    peak_drop = baseline_peak - float(ablated_metrics.get("permuted_fashion_mnist_peak_accuracy_mean", 0.0))
    forgetting_increase = float(ablated_metrics.get("permuted_fashion_mnist_mean_forgetting", 0.0)) - baseline_forgetting
    bwt_drop = float(baseline_bwt - float(ablated_metrics.get("permuted_fashion_mnist_bwt", 0.0)))
    margin_drop = baseline_margin - float(ablated_metrics.get("permuted_fashion_mnist_mean_margin", 0.0))
    ...
    z_gap_drop = baseline_z_gap - float(ablated_metrics.get("z_memory_alignment_gap", 0.0))
    growth_peak_drop = baseline_growth_peak - float(ablated_metrics.get("permuted_fashion_mnist_peak_growth_pressure_mean", 0.0))
    growth_trigger_drop = baseline_growth_trigger - float(
        ablated_metrics.get("permuted_fashion_mnist_growth_trigger_crossed_fraction", 0.0)
    )
    ...
    support_score = float(
        ...
        + (1 if margin_drop >= 0.005 else 0)
        + (1 if energy_drop >= 0.01 else 0)
        + (1 if stress_rise >= 0.01 else 0)
        + (1 if z_drop >= 0.03 else 0)
        + (1 if z_gap_drop >= 0.01 else 0)
    ...
    return {
        ...
        "z_memory_gap_drop": float(z_gap_drop),
...
def summarize_ablation_runs(condition_runs: list[dict]) -> dict:
    ...
    summary = {
        ...
        "baseline_final_accuracy_mean": float(baseline_metrics.get("permuted_fashion_mnist_final_accuracy_mean", 0.0)),
        "baseline_mean_forgetting": float(baseline_metrics.get("permuted_fashion_mnist_mean_forgetting", 0.0)),
        "baseline_bwt": float(baseline_metrics.get("permuted_fashion_mnist_bwt", 0.0)),
        "baseline_mean_margin": float(baseline_metrics.get("permuted_fashion_mnist_mean_margin", 0.0)),
    ...
    for condition_name, run in runs_by_condition.items():
        ...
        summary[f"{prefix}_final_accuracy_mean"] = float(metrics.get("permuted_fashion_mnist_final_accuracy_mean", 0.0))
        summary[f"{prefix}_mean_forgetting"] = float(metrics.get("permuted_fashion_mnist_mean_forgetting", 0.0))
        summary[f"{prefix}_bwt"] = float(metrics.get("permuted_fashion_mnist_bwt", 0.0))
        summary[f"{prefix}_mean_margin"] = float(metrics.get("permuted_fashion_mnist_mean_margin", 0.0))
        summary[f"{prefix}_peak_growth_pressure_mean"] = float(metrics.get("permuted_fashion_mnist_peak_growth_pressure_mean", 0.0))
        summary[f"{prefix}_growth_trigger_crossed_fraction"] = float(
            metrics.get("permuted_fashion_mnist_growth_trigger_crossed_fraction", 0.0)
        )
...
def build_ablation_markdown(condition_runs: list[dict], summary: dict) -> str:
    lines = [
        "# Permuted-FashionMNIST Matched Ablations",
        "",
    ...
    for run in condition_runs:
        ...
        lines.extend(
            [
                ...
                f"- peak_accuracy_mean: {metrics.get('permuted_fashion_mnist_peak_accuracy_mean', 0.0):.4f}",
                ...
                f"- z_memory_alignment_gap: {metrics.get('z_memory_alignment_gap', 0.0):.4f}",
                f"- peak_growth_pressure_mean: {metrics.get('permuted_fashion_mnist_peak_growth_pressure_mean', 0.0):.4f}",
                f"- growth_trigger_crossed_fraction: {metrics.get('permuted_fashion_mnist_growth_trigger_crossed_fraction', 0.0):.4f}",
            ]
        )
        if run["condition"] != "baseline":
            ...
            lines.extend(
                [
                    ...
                    f"- z_memory_gap_drop_vs_baseline: {summary[f'{prefix}_z_memory_gap_drop']:.4f}",
...
def run_ablation_suite(config_path: Path, conditions: tuple[str, ...], *, challenge_variant: str = "growth_probe") -> dict:
    ...
    assay = PermutedFashionMNISTAssay()
    ...
    for condition_name in conditions:
        ...
        result = assay.run(cfg) if condition_name == "baseline" else assay.run_condition(cfg, condition_name)
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/assay/permuted_fashion_mnist.yaml")
    ...
    parser.add_argument("--json-out", default="artifacts/permuted_fashion_mnist_ablations.json")
    parser.add_argument("--markdown-out", default="artifacts/permuted_fashion_mnist_ablations.md")
