...
def primary_score(metrics: dict) -> float:
    return float(
        metrics.get("sequential_rules_final_accuracy_mean", 0.0)
        - 0.45 * metrics.get("sequential_rules_mean_forgetting", 0.0)
        + 0.20 * metrics.get("sequential_rules_bwt", 0.0)
        + 0.10 * metrics.get("sequential_rules_mean_margin", 0.0)
    )
...
def biomarker_score(metrics: dict) -> float:
    return float(
        ...
        - 0.30 * metrics.get("mean_stress", 0.0)
        + 0.10 * abs(metrics.get("mean_z_alignment", 0.0))
        + 0.05 * metrics.get("mean_growth_pressure", 0.0)
    )
...
def summarize_seed_runs(seed_runs: list[dict]) -> dict:
    ...
    final_acc = values("sequential_rules_final_accuracy_mean")
    forgetting = values("sequential_rules_mean_forgetting")
    bwt = values("sequential_rules_bwt")
    margins = values("sequential_rules_mean_margin")
    ...
    summary["stable_across_seeds"] = bool(
        summary["final_accuracy"]["min"] >= 0.68
        and summary["forgetting"]["max"] <= 0.10
...
def build_seed_markdown(seed_runs: list[dict], summary: dict) -> str:
    lines = [
        "# Sequential Rules Seed Robustness",
        "",
    ...
    for item in seed_runs:
        ...
        lines.extend(
            [
                ...
                f"- final_accuracy_mean: {metrics.get('sequential_rules_final_accuracy_mean', 0.0):.4f}",
                f"- mean_forgetting: {metrics.get('sequential_rules_mean_forgetting', 0.0):.4f}",
                f"- bwt: {metrics.get('sequential_rules_bwt', 0.0):.4f}",
                f"- mean_margin: {metrics.get('sequential_rules_mean_margin', 0.0):.4f}",
                "",
            ]
        )
    return "\n".join(lines)
...
def run_seed_robustness(config_path: Path, seeds: tuple[int, ...]) -> dict:
    ...
    for seed in seeds:
        ...
        cfg.run.name = f"sequential_rules_seed_{seed}"
...
def main() -> None:
    ...
    parser.add_argument("--json-out", default="artifacts/sequential_rules_seed_robustness.json")
    parser.add_argument("--markdown-out", default="artifacts/sequential_rules_seed_robustness.md")
