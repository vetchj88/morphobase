...
from scripts.rank_split_mnist_sweeps import biomarker_score, build_markdown_report, primary_score
...
def summarize_seed_runs(seed_runs: list[dict]) -> dict:
    ...
    final_acc = values("split_mnist_final_accuracy_mean")
    forgetting = values("split_mnist_mean_forgetting")
    bwt = values("split_mnist_bwt")
    margins = values("split_mnist_mean_margin")
    ...
    summary["stable_across_seeds"] = bool(
        ...
        and summary["forgetting"]["max"] <= 0.25
...
def run_seed_robustness(config_path: Path, seeds: tuple[int, ...]) -> dict:
    ...
    for seed in seeds:
        ...
        cfg.run.name = f"split_mnist_seed_{seed}"
...
def main() -> None:
    ...
    parser.add_argument("--json-out", default="artifacts/split_mnist_seed_robustness.json")
    parser.add_argument("--markdown-out", default="artifacts/split_mnist_seed_robustness.md")
