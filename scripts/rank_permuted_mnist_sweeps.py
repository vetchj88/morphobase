...
def primary_score(metrics: dict) -> float:
    return float(
        1.00 * metrics.get("permuted_mnist_final_accuracy_mean", 0.0)
        + 0.35 * metrics.get("permuted_mnist_peak_accuracy_mean", 0.0)
        - 0.85 * metrics.get("permuted_mnist_mean_forgetting", 0.0)
        + 0.35 * metrics.get("permuted_mnist_bwt", 0.0)
        + 0.10 * metrics.get("permuted_mnist_mean_margin", 0.0)
    )
...
def main() -> None:
    ...
    parser.add_argument("--json-out", default="artifacts/permuted_mnist_sweep_ranking.json")
    parser.add_argument("--markdown-out", default="artifacts/permuted_mnist_sweep_ranking.md")
