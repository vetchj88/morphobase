...
from scripts.rank_split_fashion_mnist_sweeps import biomarker_score, build_markdown_report, primary_score
...
def summarize_seed_runs(seed_runs: list[dict]) -> dict:
    ...
    final_acc = values("split_fashion_mnist_final_accuracy_mean")
    forgetting = values("split_fashion_mnist_mean_forgetting")
    bwt = values("split_fashion_mnist_bwt")
    margins = values("split_fashion_mnist_mean_margin")
    ...
    summary["stable_across_seeds"] = bool(
        summary["final_accuracy"]["min"] >= 0.30
        and summary["forgetting"]["max"] <= 0.28
        and summary["final_accuracy"]["std"] <= 0.10
    )
    return summary
...
def build_seed_markdown(seed_runs: list[dict], summary: dict) -> str:
    lines = [
        "# Split-FashionMNIST Seed Robustness",
        "",
...
def run_seed_robustness(config_path: Path, seeds: tuple[int, ...]) -> dict:
    ...
    for seed in seeds:
        ...
        cfg.run.name = f"split_fashion_mnist_seed_{seed}"
...
def main() -> None:
    ...
    parser.add_argument("--json-out", default="artifacts/split_fashion_mnist_seed_robustness.json")
    parser.add_argument("--markdown-out", default="artifacts/split_fashion_mnist_seed_robustness.md")
